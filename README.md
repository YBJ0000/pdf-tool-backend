...
- **Dynamic font size (single-line):** If the text is too long for the field’s width, the font size is reduced (down to a minimum) so the text fits in one line. If it still overflows at the minimum size, the text is truncated with an ellipsis (`...`). Multi-line fields (tall rectangles) and automatic line wrapping are planned for a later release.
